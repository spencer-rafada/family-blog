...
export default function CreatePostForm({ preselectedAlbumId }: CreatePostFormProps) {
  ...
  useEffect(() => {
    if (preselectedAlbumId && albums.length > 0) {
      const albumExists = albums.find(album => album.id === preselectedAlbumId)
      if (albumExists) {
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-2xl">
            {preselectedAlbumId ? `Add Post to ${selectedAlbum?.name || 'Album'}` : 'Share a New Memory'}
          </CardTitle>
        </CardHeader>
        <CardContent>
