...
export function InviteAcceptForm({
  ...
  const { isAccepting: autoAccepting, error: autoAcceptError } =
    useAutoAcceptInvite(token, wasRedirected && !!user)
  ...
  const handleSignUp = () => {
    if (invite.is_shareable) {
      ...
      router.push(
        `/signup?invite_token=${token}&email=${encodeURIComponent(invite.email)}`
      )
    }
  }
    ...
    return (
      <div className='space-y-4'>
        <div className='bg-blue-50 border border-blue-200 rounded-md p-4'>
          <p className='text-sm text-blue-800'>
            {invite.is_shareable ? (
              ...
              <>
                To accept this invitation, you need to sign in or create an
                account with <strong>{invite.email}</strong>
              </>
            )}
          </p>
        </div>
        ...
        <p className='text-xs text-gray-500 text-center'>
          You&apos;ll be redirected back to complete the invitation after
          signing in.
        </p>
      </div>
    )
